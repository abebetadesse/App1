...
const profileOwnerService = {
  ...
  dashboard: {
    ...
    getDashboardData: async () => {
      try {
        const response = await profileOwnerAPI.get('/profile-owners/dashboard');
    ...
    getQuickStats: async () => {
      try {
        const response = await profileOwnerAPI.get('/profile-owners/dashboard/stats');
        return response.data;
      } catch (error) {
        throw new Error(error.response?.data?.error || 'Failed to get quick stats');
      }
    },
    ...
    getRecentActivity: async () => {
      try {
        const response = await profileOwnerAPI.get('/profile-owners/dashboard/activity');
        return response.data;
      } catch (error) {
        throw new Error(error.response?.data?.error || 'Failed to get recent activity');
      }
    },
  },
...
// Named exports for individual service categories
export const {
  dashboard,
  profile,
  documents,
...
export const mockProfileOwnerData = {
  dashboard: {
  ...
  profile: {
    ...
    bio: 'Experienced full-stack developer with 5+ years in web and mobile development.',
    isVerified: true,
    isAvailable: true
  }
};